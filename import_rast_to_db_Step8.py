...
for file_name in list_files:
    ...
    print(file_name)
    if len(taxo) == 7 or 'Acinetobacter_baumannii' in file_name.lower():
        ...
        if 'acinetobacter_calcoaceticus' in file_name.lower():
            family_obj = Family(designation = 'Moraxellaceae')
            genus_obj = Genus(designation = 'Acinetobacter')
            specie_obj = Specie(designation = 'Acinetobacter calcoaceticus')
            name_bacterium_strain = file_name.split('-')[1]
            name_bacterium_strain = name_bacterium_strain.split('_')[2]
