...
from files_treatment_new.fasta_contigs_NCBI import FastaContigsNCBI
...
path = dir_path + '/NCBI/organisms/NZ_AHDR00000000.1.Contig.fasta'
print(path)

path_proteins = dir_path + '/NCBI/organisms/NZ_AHDR00000000.1.prot_aa.fasta'
path_proteins_CDS = dir_path + '/NCBI/organisms/NZ_CP016896.1.prot_csd.fasta'

fasta_protein_obj_dict = Fasta_protein_NCBI(path_proteins, 'db_xref')
...
contig_Fasta_obj = FastaContigsNCBI(path)
...
for key, value_contig in contig_Fasta_obj.dict_fasta_data.items():
    contig_id = value_contig.id
    for key, value in fasta_protein_obj_dict.dict_fasta_data.items():
        if re.search(value_contig.id, value.description, re.IGNORECASE):
...
#id_bacterium = createBacterium('NZ_AHDR00000000.1', person_responsible, source_data, id_strain)
