...
def generate_PPI(couple_obj):
    print("Couple tested {0}".format(couple_obj.id_couple))
    global list_couples
    ...
    lock.release()
    print("END Couple tested {0}".format(couple_obj.id_couple))
...
with ThreadPoolExecutor(max_workers=max_proce) as executor:
    futures = [executor.submit(generate_PPI, couple_obj) for couple_obj in list_couples]
    for future in as_completed(futures):
        print(future.result())
